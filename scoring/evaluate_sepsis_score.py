...
def evaluate_utility(patients, cohort_labels, cohort_predictions):
    ...
    return np.round(normalized_observed_utility,4)

# The compute_prediction_utility function computes the total time-dependent
# utility for a patient.
def compute_prediction_utility(labels, predictions, dt_early=-8, dt_optimal=-4, dt_late=3.0, max_u_tp=1, min_u_fn=-2, u_fp=-0.05, u_tn=0, check_errors=True):
